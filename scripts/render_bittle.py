#!/usr/bin/env python3
"""
MH-FLOCKE — Bittle MuJoCo Video Renderer (with Dashboard)
==============================================================
Renders Bittle training FLOG with full dashboard overlay.
Based on render_go2_mujoco.py pipeline.

Usage:
    python scripts/render_bittle.py creatures/bittle/v043_.../training_log.bin
...
def render_title_card(w, h, fps, dur, pipe, meta=None):
    ...
    meta = meta or {}
    version = VERSION  # single source (was meta.get('version', 'v0.4.3'))
    ...
    for i in range(n):
        ...
        d.text((w//2-ox, cy-int(15*sc)), f'Petoi Bittle X \u00b7 Level 15 {version}',
        ...
        lines = [
            ...
            'Robot: Petoi Bittle X \u00b7 BiBoard V0 \u00b7 ESP32',
            f'{n_neurons} neurons \u00b7 8 actuators \u00b7 {total_steps:,} steps',
            '',
...
def render_end_card(w, h, fps, dur, pipe, stats):
    ...
    for i in range(n):
        ...
        a = lambda v: int(v * alpha)
        cy = h // 2
        d.text((w//2-ox, cy-int(110*sc)), 'MH-FLOCKE \u2014 Bittle X',
               fill=(0, a(200), a(220)), font=fb)
...
def main():
    p = argparse.ArgumentParser(description='MH-FLOCKE Bittle Renderer')
    ...
    if args.output is None:
        args.output = os.path.join(os.path.dirname(args.flog), 'bittle_render.mp4')

    print(f'\n{"="*60}')
    print(f'  MH-FLOCKE \u2014 Bittle Renderer (with Dashboard)')
    print(f'{"="*60}')
    ...
    xml_path = os.path.abspath(BITTLE_XML)
    if _scene_has_wall:
    ...
    # ambient, so the robot/floor render dark. Lift the headlight ambient/diffuse.
    ...
    opt.geomgroup[1] = False  # Hide collision geoms (group 1 = gray overlay)
    ...
    for vf in range(n_frames):
        ...
        if dash:
            ...
            stats['creature'] = flog.meta.get('creature', 'bittle').capitalize()
            stats['version'] = VERSION  # single source (was hardcoded 'v0.4.8')
